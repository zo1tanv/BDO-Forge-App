...
Приложение обновляется само. Оно раз в несколько часов сверяется с подписанным манифестом, берёт
установщик из выпуска здесь (а если GitHub недоступен — с bdo-forge.com) и перед запуском проверяет
подпись выпуска и контрольную сумму. Файлы отсюда нужны только для первой установки — следить за
выпусками вручную не требуется.

Сайт тоже следит за этим репозиторием: страница [bdo-forge.com/app](https://bdo-forge.com/app)
берёт номер версии и историю изменений из последнего выпуска, так что оба места всегда показывают
одно и то же.
